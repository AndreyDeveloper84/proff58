// BFF: GET /api/account/wishlist/products?ids=1,2,3 → карточки товаров избранного.
//
...
//
// Почему под /api/account/, а не рядом с каталогом: nginx уводит весь
// /api/catalog/ прямо в Django (кроме поимённых исключений), и роут по такому
// адресу до Next просто не доезжает — 404 в проде при зелёной локальной сборке.
// Префикс /api/account/ проксируется во фронт целиком, поэтому новый маршрут
// работает и без правки конфигурации nginx, и не может с ней разойтись.
