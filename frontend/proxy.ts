...
export const config = {
  // Весь кабинет, кроме формы входа (иначе редирект зациклится) и старого адреса
  // избранного: оно переехало на витрину и входа не требует, а страница по
  // прежнему адресу просто уводит на /wishlist.
  matcher: ["/account", "/account/((?!login|wishlist).*)"],
};
