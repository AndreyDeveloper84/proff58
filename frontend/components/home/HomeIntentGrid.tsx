  ...
  return (
    <section className="bg-canvas" aria-labelledby="intent-title">
      <div className="mx-auto w-full max-w-[1680px] px-4 pt-4 sm:px-6 xl:px-8">
        <h2 id="intent-title" className="mb-2 font-sans text-xl font-bold text-ink">
          {title}
        </h2>
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-5">
          {cards.map((c) => (
            <Link
              key={c.title}
              href={c.href}
              className="group flex min-h-[104px] items-center gap-3 rounded-sm border border-line bg-card px-3 py-2 shadow-card transition hover:border-accent hover:shadow-md"
            >
              <Image
                src={c.image}
                alt=""
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1 block text-[13px] leading-[1.35] text-ink-2">{c.text}</span>
              </span>
            </Link>
          ))}
        </div>
