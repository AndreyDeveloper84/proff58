...
/** maxEnabled — бот MAX настроен на сервере (max_bot_url непустой). Без него
 * кнопку «Войти через MAX» не показываем: она давала бы только ошибку 503. */
export function LoginForm({ maxEnabled = true }: { maxEnabled?: boolean }) {
        ...
        <aside className="border-t border-line bg-accent/5 p-5 sm:p-7 lg:border-l lg:border-t-0 lg:p-8">
          ...
          <div className="mt-6 space-y-5">
            {[
              ...
              ...(maxEnabled ? [[Bell, "Уведомления в MAX"]] : []),
            ].map(([Icon, label]) => {
