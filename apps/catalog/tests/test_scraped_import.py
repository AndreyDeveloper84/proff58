...
def test_power_hp_and_power_are_separate_axes_in_the_dictionary():
    ...
    # Раньше здесь стояло «ни один блок не объявляет обе оси сразу». Запрет
    # защищал ФИЛЬТР: две мощности в разных единицах в одном фасете — это 1,8 и
    # 1320 в одном списке значений.
    #
    # Решение владельца 2026-09-10 «кВт, охват важнее» потребовало дать
    # бензоинструменту фасет мощности в киловаттах: «л.с.» есть у 65 товаров,
    # «кВт» — у 130. Поэтому блок теперь может объявлять обе оси, но ровно на
    # одном условии: у power_hp снят фасет (`bind: false`). Ось живёт в карточке
    # товара, в фильтр не попадает, и смешаться значениям негде.
    #
