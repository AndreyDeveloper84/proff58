...
def test_seed_reproduces_canonical_identity_hash(seeded):
    ...
    assert AttributeOption.objects.filter(attribute__slug="tool_type").count() == 361
...
def test_downgrade_drops_four_unused_options_and_lands_on_target_identity(seeded, tmp_path):
    ...
    assert AttributeOption.objects.filter(attribute__slug="tool_type").count() == 357
