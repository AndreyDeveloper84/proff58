...
def test_no_battery_does_not_glue_next_word():
    """«без аккум.и зар. устройства» — точка тут ещё и разделитель слов.

    Без пробела получалось «без аккумулятораи зар. устройства» — на стенде так
    вышло у трёх фонарей BOSCH.
    """
    assert normalize_name("Фонарь BOSCH GLI 14,4 V-LI (без аккум.и зар. устройства)") == (
        "Фонарь BOSCH GLI 14,4 V-LI (без аккумулятора и зар. устройства)"
    )
    # Обычный случай с пробелом не должен получить второй пробел.
    assert normalize_name("Винтоверт аккум. ЗУБР без аккум. и ЗУ") == (
        "Винтоверт аккумуляторный ЗУБР без аккумулятора и ЗУ"
    )
