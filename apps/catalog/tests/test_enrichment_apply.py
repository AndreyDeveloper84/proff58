...
def _product(**kw):
    ...
    data = dict(
        ...
        name="",
        slug="p1",
        original_name="Перфоратор HR2470 Makita",
...
def test_fills_only_empty_card_fields():
    ...
    res = apply_ai_enrichment(
        p, name="Перфоратор Makita HR2470", description="новое", confidence=0.9
    )
    p.refresh_from_db()
...
def test_llm_does_not_overwrite_manual_pav():
    ...
    ProductAttributeValue.objects.create(
        product=p, attribute=attr, value_integer=900, source=Source.MANUAL, confidence=100
    )
...
def test_force_overwrites_existing_card_field():
    ...
    res = apply_ai_enrichment(
        p, name="Новое имя Makita", description="Новое описание PRO", confidence=0.9, force=True
    )
    p.refresh_from_db()
