...
def pending_for_enrichment(
    *, category_slug: str | None = None, limit: int = 100, only_empty: bool = True
    ...
    return list(qs.order_by("-available_quantity", "id").values_list("id", flat=True)[:limit])
...
def _apply_attributes(product: Product, attributes: list[AiAttr]) -> list[str]:
    ...
    for ai_attr in attributes:
        ...
        if attr is None or attr.attribute_type in (AttributeType.SELECT, AttributeType.MULTISELECT):
            continue
        existing = ProductAttributeValue.objects.filter(product=product, attribute=attr).first()
...
def apply_ai_enrichment(
    ...
    *,
    ...
    return {
        "locked": False,
        "fields_updated": fields + attr_updated,
