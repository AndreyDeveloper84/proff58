...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for attr in attrs:
                totals["scanned"] += 1
                self._process_attribute(attr, dry_run, totals)
            if dry_run:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                "Options already had slug: {already} | Skipped empty values: {skipped_empty}".format(
    ...
    def _process_attribute(self, attr: Attribute, dry_run: bool, totals: dict) -> None:
