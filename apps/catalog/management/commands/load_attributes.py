...
задаёт: у оси адрес приоритетнее блочного. Ось с ``bind: false`` привязки не
получает вовсе — это явно записанное решение «фасета здесь не будет».
        ...
        *,
        ...
        for tt in data.get("tool_types", []):
            ...
            for a in tt.get("attributes", []):
                ...
                # `bind: false` — у оси фасета НЕ будет, и это решение, а не
                # недосмотр. Без явного флага вариант «оставить без фасета»
                # (ДРФ-1524, развилка В) выразить в словаре нечем: ось вечно
                # висит в плане как несозданная привязка, и отличить «решили не
                # делать» от «забыли» невозможно — из-за этого проверку
                # привязок нельзя сделать фатальной.
                binding_category = a.get("category") or category_name
                if a.get("bind") is False:
