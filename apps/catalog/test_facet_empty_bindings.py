...
# Числовые оси, объявленные без шаблонов извлечения. Список закрыт намеренно —
# он фиксирует долг, а не разрешает его пополнять.
#
# Долг по `weight_kg` разобран замером по каждому типу, и типы разошлись:
# у дрелей, перфораторов и шлифмашин «кг» — это вес инструмента, шаблон написан.
# У ДОМКРАТОВ «кг» означает ГРУЗОПОДЪЁМНОСТЬ («Стеклодомкрат SP-200 110кг»,
# «Пневмоподушка 170кг»), поэтому шаблона там нет и не будет: он писал бы в «Вес»
# чужую величину. Грузоподъёмность ведёт ось `capacity` (тонны, 135 значений), а
# пять настоящих весов домкратов внесены вручную.
...
# --- weight_kg: один и тот же «кг» означает разное ----------------------------


@pytest.mark.parametrize(
    ("tt", "name", "kg"),
    [
        ("dreli-shurupoverty", "Дрель D 6SH 240ВТ, 4500об/мин, 0,9кг", "0.9"),
        ("perforatory", "Перф. DH40MC; 1100Вт, SDS-max, 10,5Дж, 2800уд/мин, 7,1кг", "7.1"),
        ("shlifmashiny", "Шлифмаш прямая GP2S2; 520 Вт, цанга-6 мм, 25000 об/мин, 1,8 кг", "1.8"),
    ],
)
def test_tool_weight_is_read(rules, tt, name, kg):
    """У электроинструмента «кг» в названии — вес самого инструмента."""
    got = next((v.number for v in rules.extract(tt, name) if v.slug == "weight_kg"), None)
    assert got == Decimal(kg)


@pytest.mark.parametrize(
    "name",
    [
        "Стеклодомкрат вакуумный KRAFTOOL SP-200.ф200мм 110кг",
        "Пневмоподушка ЗУБР набор 3шт 12х19,16х15,13х28см 170кг в чехле",
        "Домкрат 16т гидравлический KRAFTOOL 230-460мм",
    ],
)
def test_jack_capacity_never_becomes_weight(rules, name):
    """У домкратов «кг» — это ГРУЗОПОДЪЁМНОСТЬ, и ось веса обязана молчать.

    Стеклодомкрат на присосках держит 110 кг, а весит меньше килограмма. Шаблон
    здесь написал бы в «Вес» чужую величину, поэтому его нет и не будет:
    грузоподъёмность ведёт ось `capacity` (тонны), а пять настоящих весов
    домкратов внесены вручную и regex их не касается.
    """
    got = next((v.number for v in rules.extract("domkraty", name) if v.slug == "weight_kg"), None)
    assert got is None


def test_model_number_is_not_a_weight(rules):
    """«ДУ-13/580Т» — индекс модели, а не 580 тонн и не вес."""
    name = 'Дрель удар. ДУ-13/580Т (580Вт) "Интерскол"'
    got = next(
