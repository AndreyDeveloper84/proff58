...
def test_карточки_разбираются_в_пары():
    ...
    assert sections[0]["items"] == [
        {"title": "Самовывоз", "text": "Заберите на 1-м Онежском, 12", "image": ""},
        {"title": "Курьер", "text": "Привезём в день заказа", "image": ""},
    ]
...
def test_пункт_без_разделителя_не_теряется():
    ...
    assert sections[0]["items"] == [{"title": "Свяжитесь с нами", "text": "", "image": ""}]
...
def test_у_карточки_может_быть_иллюстрация():
    sections = parse_page_body(
        "## Способы получения\n:карточки\n"
        "- Самовывоз | Заберите на складе | /info/delivery/pickup.webp\n"
        "- Курьер | Привезём по городу\n"
    )

    # Картинка — третья часть той же строки: отдельный синтаксис ради неё
    # человеку осваивать не нужно, а карточка без картинки остаётся рабочей.
    assert sections[0]["items"][0]["image"] == "/info/delivery/pickup.webp"
    assert sections[0]["items"][1]["image"] == ""
