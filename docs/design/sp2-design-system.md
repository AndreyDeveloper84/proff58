...
## Стратегия темы (SP2.1, #474)

**Light-first**: дефолтные значения токенов (`:root` / `[data-theme="light"]`) —
светлые; предназначены для рабочих e-commerce поверхностей (каталог, PDP, checkout, ЛК).
Тёмная тема — `[data-theme="dark"]`; ею оформляются header/footer/маркетинговые секции.
Тему можно переключать на любом поддереве: ближайший предок с `data-theme` выигрывает
при резолве CSS-переменных, поэтому светлый каталог и тёмный маркетинг сосуществуют
на одной странице.

Текущая витрина исторически тёмная — на корне (`<html>`) стоит `data-theme="dark"`,
поэтому live-страницы визуально не меняются. Миграция товарных поверхностей на светлую
тему выполняется по мере вёрстки реальных экранов (оборачиванием в `data-theme="light"`
или сменой корня) — отдельно, с визуальным QA (вне объёма SP2.1).

Компоненты используют **только семантические токены** → переключение темы их не трогает.
