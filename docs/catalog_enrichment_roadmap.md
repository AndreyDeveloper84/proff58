...
- ~~**Model-pattern extraction (disc_diameter/power).**~~ ✅ Сделано (#166): строго-guarded
  паттерны для кода модели («УШМ-125/1100», «УШМ-П230-2000», AG125-1500) — канонические Ø,
  ватты 400–2999 по разделителю. disc 47%→97%, power 47%→90%, значения чистые.
- ~~**power_source для сетевых.**~~ ✅ Сделано (#168): декларативный `derive`-механизм движка,
  источник `inferred` (prio 10). На staging power_source у УШМ 1%→92%, 0 ложных. Подключено к
  УШМ/перфораторам/дрелям. (#169 — дедуп правила: было два правила с одним slug.)
- ~~**Идемпотентный `enrich` (само-починка).**~~ ✅ Сделано (#172): при re-enrich удаляются
  engine-значения (`regex/keyword/inferred`) управляемых атрибутов, которых движок больше не
  извлекает, + чистка `attrs_cache`; `manual`/`import_1c`/`llm` неприкосновенны. Ручная чистка
  устаревших `voltage>99` больше не нужна.
