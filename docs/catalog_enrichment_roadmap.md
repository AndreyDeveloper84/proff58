...
**Тираж по типам (Электроинструмент):** дрели (каркас) → **перфораторы (#160, сделано)** →
далее болгарки/УШМ, лобзики и т.д. — по одному типу на PR.

**Follow-up движка (отложено, выявлено на перфораторах):**
- **power_source для сетевых.** Сетевой инструмент не пишет «сетев/220в» в имени → питание не
  размечается (у перфораторов 13%). Инференс «есть `power`(Вт) и нет аккум/вольт → Сеть».
  Реализовать как ОБЩЕЕ правило движка (не спец-кейс одного типа) — поднимет покрытие питания
  у всего сетевого инструмента.
