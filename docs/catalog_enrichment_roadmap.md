...
- **power_source для сетевых.** Сетевой инструмент не пишет «сетев/220в» в имени → питание не
  размечается. Инференс «есть `power`(Вт) и нет аккум/вольт → Сеть». Реализовать как ОБЩЕЕ
  правило движка (не спец-кейс одного типа).
