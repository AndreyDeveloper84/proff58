...
### Лок деплоя и post-deploy ассерты (#576)

Деплой держит эксклюзивный лок `DEPLOY_PATH/.deploy.lock` (flock). **Любые ручные
compose-операции на стенде выполнять только под тем же локом**, иначе коллизия
с идущим деплоем оставит стенд в рассинхроне:

```bash
cd <DEPLOY_PATH>
flock .deploy.lock docker compose -f docker-compose.prod.yml <команда>
# release.sh берёт лок сам (ждёт до 5 минут)
```

В конце деплоя выполняются ассерты (провал любого = failed run с точной причиной):
1. SHA кода в работающем web (`/app/.build-sha`) == checkout — web пересоздан из свежего образа;
2. `migrate --check` внутри web — схема БД синхронна коду;
3. `healthz` изнутри web — 200.

Проверка завершённых ранов: `gh run view <id> --exit-status` (⚠️ `gh run watch
--exit-status` на УЖЕ завершённом ране возвращает 0 даже при failure — баг gh ≤2.95).
