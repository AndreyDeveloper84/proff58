...
### 9.4 Процесс Architecture Decision Records (ADR)

Ключевые решения фиксируются в `docs/adr/` в виде ADR-файлов. Шаблон — `ADR-0000-template.md`.

**Правило:** ADR обязателен при:
- изменении публичного контракта сервиса (`services.py` → тип/имя параметра/возвращаемого значения);
- добавлении или изменении payload доменного сигнала (`apps/core/events.py`);
- изменении схемы API (добавление обязательного поля, удаление поля, смена формата);
- любом архитектурном решении, которое будет непонятно «новому разработчику через год».
