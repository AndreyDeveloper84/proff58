...
### Повторная сверка внутри транзакции записи (H6)

План строится **вне** транзакции записи: между `plan` и `apply` есть окно, в котором
чужой процесс может поменять `tool_type`. Поэтому таблица решений применяется дважды —
один раз при построении плана и второй раз внутри транзакции `apply_rollback`, тем же
решающим правилом (`_decide`, общая точка для обоих контуров). Перед повторной сверкой
строки `Product` и `ProductAttributeValue` берутся под `SELECT … FOR UPDATE`
(упорядоченно по id — против взаимных дедлоков двух откатов).
...
| товар удалён | отказ: `товар N исчез между планом и применением` |
| целевая опция удалена | отказ: `option 'X' исчез между планом и применением` |

Отказ поднимается **до** первой записи, поэтому план не применяется частично.

> **Остаточный риск.** `FOR UPDATE` блокирует существующие строки, но не мешает
> чужой транзакции **вставить** отсутствовавший `ProductAttributeValue`. Такая вставка,
> закоммиченная до повторной сверки, будет увидена и даст conflict; закоммиченная после —
> нет. Полная сериализация потребовала бы уровня изоляции `SERIALIZABLE` и здесь
> сознательно не вводится: все штатные писатели `tool_type` в репозитории проходят
> через `Product.attrs_cache` и потому конкурируют за ту же блокировку.
