...
1. в блоке `str-skoby` **добавить обратно** правило `piece_count` с рабочим
   regex фасовки `(?<!\d)(\d{3,5})\s*шт` и без `skip_if`. Именно добавить, а не
   отредактировать: миграция выполнена, и по контракту §6 transitional-правило
   из постоянного ruleset удалено — в блоке остались только `length` и
   `package_quantity`;
