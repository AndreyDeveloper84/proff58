...
Цены не извлекаются. Сам клиент качает только текст страниц; адреса фотографий
он лишь отдаёт в карточке (``ProductCard.images``), скачиванием файлов
занимается отдельный контур (трек ИЗО) — но декларация в User-Agent честно
говорит источнику, что изображения мы забираем.
...
# Честный User-Agent с контактом (НЕ маскировка под браузер). Версия 0.2:
# владелец принял решение собирать фотографии товаров, и прежняя декларация
# «no images» перестала быть правдой — UA обязан описывать реальное поведение.
# Контакт и заявленный темп сохранены.
USER_AGENT = (
